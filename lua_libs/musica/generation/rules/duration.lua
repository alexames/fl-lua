...
DurationRangeRule = class('DurationRangeRule'):extends(Rule)({
  ...
  validate = function(self, figure)
    for i, note in ipairs(figure.notes) do
      if note.duration < self.min_duration then
        return false,
          string.format(
            'Note %d duration %.3f below minimum %.3f',
            i,
            note.duration,
            self.min_duration
          )
      ...
      if note.duration > self.max_duration then
        return false,
          string.format(
            'Note %d duration %.3f above maximum %.3f',
            i,
            note.duration,
            self.max_duration
          )
  ...
  __tostring = function(self)
    return string.format(
      'DurationRangeRule{min=%.3f, max=%.3f}',
      self.min_duration,
      self.max_duration
    )
  end,
...
TotalDurationRule = class('TotalDurationRule'):extends(Rule)({
  ...
  validate = function(self, figure)
    ...
    if self.exact_total then
      ...
      if self.min_total and total < self.min_total then
        return false,
          string.format(
            'Total duration %.3f below minimum %.3f',
            total,
            self.min_total
          )
      end
      if self.max_total and total > self.max_total then
        return false,
          string.format(
            'Total duration %.3f above maximum %.3f',
            total,
            self.max_total
          )
      end
  ...
  __tostring = function(self)
    if self.exact_total then
      ...
      return string.format(
        'TotalDurationRule{min=%s, max=%s}',
        self.min_total or 'nil',
        self.max_total or 'nil'
      )
    end
  end,
})
...
FixedDurationRule = class('FixedDurationRule'):extends(Rule)({
  ...
  validate = function(self, figure)
    ...
    if math.abs(note.duration - self.duration) > 0.001 then
      return false,
        string.format(
          'Note %d duration %.3f != required %.3f',
          self.index,
          note.duration,
          self.duration
        )
  ...
  __tostring = function(self)
    return string.format(
      'FixedDurationRule{index=%d, duration=%.3f}',
      self.index,
      self.duration
    )
  end,
