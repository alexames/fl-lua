...
TestLogger = class('TestLogger')({
  ...
  class_preamble = function(test_suite)
    printf(
      '%s[==========]%s Running %s tests from %s%s%s',
      color(green),
      reset(),
      #test_suite:tests(),
      color(bright_cyan),
  ...
  finale = function(total_failure_count, total_test_count)
    if total_failure_count == 0 then
      printf('%s[==========]%s All tests succeeded!', color(green), reset())
    else
      printf(
        '%s[==========]%s %s / %s failed.',
        color(red),
...
HierarchicalLogger = class('HierarchicalLogger')({
  ...
  test_end = function(test_suite, test_name, successful, err)
    ...
    if successful then
      current_hierarchical_logger.current_suite.passed = current_hierarchical_logger.current_suite.passed
        + 1
      current_hierarchical_logger.total_passed = current_hierarchical_logger.total_passed
        + 1
    else
      current_hierarchical_logger.current_suite.failed = current_hierarchical_logger.current_suite.failed
        + 1
      current_hierarchical_logger.total_failed = current_hierarchical_logger.total_failed
        + 1
    end
    current_hierarchical_logger.total_tests = current_hierarchical_logger.total_tests
      + 1
  end,
