...
SchemaException = class('SchemaException'):extends(Exception)({
  __init = function(self, path, failure_reason, level)
    local path_str = #path > 0 and ('`' .. table.concat(path, '.') .. '`')
      or 'root'
    local what = string.format('error at %s: %s', path_str, failure_reason)
...
SchemaMissingFieldException = class('SchemaMissingFieldException'):extends(
  SchemaException
)({
  __init = function(self, path, field_key, level)
    local failure_reason = string.format('missing required field %s', field_key)
    SchemaException.__init(self, path, failure_reason, (level or 1) + 1)
  end,
