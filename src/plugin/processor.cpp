...
FLLuaProcessor::FLLuaProcessor() {
    setControllerClass(kControllerUID);
}

FLLuaProcessor::~FLLuaProcessor() = default;

Steinberg::tresult PLUGIN_API FLLuaProcessor::initialize(Steinberg::FUnknown* context) {
...
Steinberg::tresult PLUGIN_API FLLuaProcessor::setActive(Steinberg::TBool state) {
    if (state) {
...
Steinberg::tresult PLUGIN_API FLLuaProcessor::canProcessSampleSize(Steinberg::int32 symbolicSampleSize) {
    if (symbolicSampleSize == Steinberg::Vst::kSample32)
        return Steinberg::kResultTrue;
    return Steinberg::kResultFalse;
}

Steinberg::tresult PLUGIN_API FLLuaProcessor::process(Steinberg::Vst::ProcessData& data) {
...
void FLLuaProcessor::updateTransport(Steinberg::Vst::ProcessData& data) {
    ...
    m_transport.playing = (ctx->state & Steinberg::Vst::ProcessContext::kPlaying) != 0;
    m_transport.sampleRate = data.processContext->sampleRate;

    if (m_transport.timeSigNum > 0) {
        m_transport.bar = static_cast<int>(m_transport.beat / m_transport.timeSigNum);
    }
...
Steinberg::tresult PLUGIN_API FLLuaProcessor::setState(Steinberg::IBStream* state) {
...
Steinberg::tresult PLUGIN_API FLLuaProcessor::getState(Steinberg::IBStream* state) {
    if (!state) return Steinberg::kResultFalse;
...
Steinberg::tresult PLUGIN_API FLLuaProcessor::notify(Steinberg::Vst::IMessage* message) {
    ...
    if (strcmp(message->getMessageID(), "ScriptSource") == 0) {
        ...
        if (message->getAttributes()->getBinary("source", data, size) == Steinberg::kResultOk) {
    ...
    if (strcmp(message->getMessageID(), "LuaLibsPath") == 0) {
        const void* data = nullptr;
        Steinberg::uint32 size = 0;
        if (message->getAttributes()->getBinary("path", data, size) == Steinberg::kResultOk) {
