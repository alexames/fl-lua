...
FLLuaPlugView::FLLuaPlugView(FLLuaController* controller)
    : CPluginView(nullptr), m_controller(controller) {
  rect.right = kDefaultWidth;
  rect.bottom = kDefaultHeight;
}

FLLuaPlugView::~FLLuaPlugView() { removed(); }

Steinberg::tresult PLUGIN_API
FLLuaPlugView::isPlatformTypeSupported(Steinberg::FIDString type) {
...
Steinberg::tresult PLUGIN_API
FLLuaPlugView::attached(void* parent, Steinberg::FIDString type) {
  if (strcmp(type, Steinberg::kPlatformTypeHWND) != 0)
    return Steinberg::kResultFalse;
  ...
  m_hwnd = CreateWindowExW(
      0, kWindowClassName, L"FL-Lua", WS_CHILD | WS_VISIBLE, 0, 0,
      kDefaultWidth, kDefaultHeight, parentHwnd, nullptr, wc.hInstance, this);
...
Steinberg::tresult PLUGIN_API
FLLuaPlugView::getSize(Steinberg::ViewRect* size) {
...
Steinberg::tresult PLUGIN_API
FLLuaPlugView::onSize(Steinberg::ViewRect* newSize) {
...
LRESULT CALLBACK FLLuaPlugView::wndProc(HWND hwnd, UINT msg, WPARAM wParam,
                                        LPARAM lParam) {
  if (ImGui_ImplWin32_WndProcHandler(hwnd, msg, wParam, lParam)) return true;

  auto* view =
      reinterpret_cast<FLLuaPlugView*>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
...
void FLLuaPlugView::renderFrame() {
  ...
  ImGui::SetNextWindowSize(ImVec2(static_cast<float>(clientRect.right),
