...
bool DX11Context::init(HWND hwnd, int width, int height) {
  ...
  HRESULT hr = D3D11CreateDeviceAndSwapChain(
      ...
      featureLevelArray, 2, D3D11_SDK_VERSION, &sd, m_swapChain.GetAddressOf(),
...
void DX11Context::resize(int width, int height) {
  ...
  cleanupRenderTarget();
  m_swapChain->ResizeBuffers(0, static_cast<UINT>(width),
...
void DX11Context::cleanupRenderTarget() { m_renderTargetView.Reset(); }
